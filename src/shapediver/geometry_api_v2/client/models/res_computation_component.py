...
class ResComputationComponent(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        _items = []
        if self.computed:
            for _item_computed in self.computed:
                _items.append(_item_computed.to_dict() if _item_computed is not None else None)
            _dict['computed'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in computing (list)
        _items = []
        if self.computing:
            for _item_computing in self.computing:
                _items.append(_item_computing.to_dict() if _item_computing is not None else None)
