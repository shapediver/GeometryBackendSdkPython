...
class ResModelComputation(BaseModel):
    ...
    timestamp_str: Annotated[str, Field(strict=True)] = Field(description="Timestamp.", json_schema_extra={"examples": ["20240521140735123"]})
    ...
    timestamp_fin: Annotated[str, Field(strict=True)] = Field(description="Timestamp when the computation request was finished.", json_schema_extra={"examples": ["20240521140735123"]})
    timestamp_req: Annotated[str, Field(strict=True)] = Field(description="Timestamp when the work request was filed.", json_schema_extra={"examples": ["1708585395633"]})
    timestamp_req_iso: Annotated[str, Field(strict=True)] = Field(description="Timestamp when the computation request was filed.", json_schema_extra={"examples": ["20240521140735123"]})
    timestamp_resp: Annotated[str, Field(strict=True)] = Field(description="Timestamp when the computation request was picked up.", json_schema_extra={"examples": ["20240521140735123"]})
    ...
    session_id: Optional[StrictStr] = Field(default=None, description="ID of the session that triggered this computation.  The value may be redacted, or omitted when the computation is not associated with a session.", alias="sessionId")
    charge_user_id: Optional[StrictStr] = Field(default=None, description="Identifier of the user billed for this computation. This may differ from the user who requested it.", alias="chargeUserId")
    charge_org_id: Optional[StrictStr] = Field(default=None, description="Identifier of the organization billed for this computation. This may differ from the organization of the user who requested it.", alias="chargeOrgId")
    __properties: ClassVar[List[str]] = ["id", "timestamp", "timestamp_str", "compute_request_id", "exports", "outputs", "params", "stats", "status", "timestamp_fin", "timestamp_req", "timestamp_req_iso", "timestamp_resp", "msg", "limit", "credits", "sessionId", "chargeUserId", "chargeOrgId"]

    @field_validator('timestamp_str', mode="before")
    ...
    @field_validator('timestamp_fin', mode="before")
    ...
    @field_validator('timestamp_req', mode="before")
    def timestamp_req_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if isinstance(value, str) and not re.match(r"^\d{13}$", value):
            raise ValueError(r"must validate the regular expression /^\d{13}$/")
        return value

    @field_validator('timestamp_req_iso', mode="before")
    ...
    @field_validator('timestamp_resp', mode="before")
